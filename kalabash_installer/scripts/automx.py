...
class Automx(base.Installer):
    ...
    def get_template_context(self):
        ...
        sql_dsn = "{}://{}:{}@{}/{}".format(
            self.dbengine,
            self.config.get("kalabash", "dbuser"),
            self.config.get("kalabash", "dbpassword"),
            self.dbhost,
            self.config.get("kalabash", "dbname"))
    ...
    def _setup_venv(self):
        ...
        if os.path.exists(target):
            os.unlink(target)
        utils.exec_cmd(
            "wget https://github.com/amonak/automx/archive/master.zip",
