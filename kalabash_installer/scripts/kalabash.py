...
class Kalabash(base.Installer):
    """Kalabash installation."""
    ...
    config_files = [
        "crontab=/etc/cron.d/kalabash",
        "sudoers=/etc/sudoers.d/kalabash",
    ]
    with_db = True
    with_user = True
    ...
    def _setup_venv(self):
        ...
        # Temp fix for https://github.com/amonak/kalabash-installer/issues/197
        python.install_package(
            kalabash_package, self.venv_path,
        ...
        if sys.version_info.major == 2 and sys.version_info.micro < 9:
            ...
            packages += ["pyOpenSSL"]
        # Temp fix for https://github.com/amonak/kalabash/issues/2247
    ...
    def _deploy_instance(self):
        ...
        if os.path.exists(target):
            ...
            if condition:
                utils.printcolor(
                    "Target directory for Kalabash deployment ({}) already "
        ...
        args = [
            "--collectstatic",
            "--timezone", self.config.get("kalabash", "timezone"),
        ...
        code, output = utils.exec_cmd(
            "bash -c '{} kalabash-admin.py deploy instance {}'".format(
                prefix, " ".join(args)),
    ...
    def get_template_context(self):
        ...
        context = super(Kalabash, self).get_template_context()
        extensions = self.config.get("kalabash", "extensions")
        ...
        context.update({
            ...
            "radicale_enabled": (
                "" if "kalabash-radicale" in extensions else "#"),
