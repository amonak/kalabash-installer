...
class Dovecot(base.Installer):
    ...
    def get_template_context(self):
        ...
        context.update({
            ...
            "kalabash_user": self.config.get("kalabash", "user"),
            "kalabash_dbname": self.config.get("kalabash", "dbname"),
            "kalabash_dbuser": self.config.get("kalabash", "dbuser"),
            "kalabash_dbpassword": self.config.get("kalabash", "dbpassword"),
            "protocols": protocols,
    ...
    def post_run(self):
        ...
        if self.dbengine == "postgres":
            dbname = self.config.get("kalabash", "dbname")
            dbuser = self.config.get("kalabash", "dbuser")
            dbpassword = self.config.get("kalabash", "dbpassword")
            backend = database.get_backend(self.config)
            backend.load_sql_file(
                dbname, dbuser, dbpassword,
                self.get_file_path("install_kalabash_postgres_trigger.sql")
            )
            backend.load_sql_file(
                dbname, dbuser, dbpassword,
                self.get_file_path("fix_kalabash_postgres_schema.sql")
            )
